...
import { Star, Calendar, Users, PenSquare } from 'lucide-react'
...
export function GameCard({ game, className }: GameCardProps) {
  ...
  return (
    <div className={cn(
      'group bg-background border rounded-lg overflow-hidden shadow-sm hover:shadow-md transition-all duration-300 hover:-translate-y-1 relative',
      ...
      <div className="relative aspect-[3/4] overflow-hidden bg-muted">
        ...
        {/* Write Review Button - Only show for authenticated users */}
        {session && (
          <div className="absolute bottom-2 left-2 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
            <Link
              href={`/games/${game.slug}/review`}
              className="inline-flex items-center space-x-1 bg-primary text-primary-foreground px-3 py-1.5 rounded-full text-xs font-medium hover:bg-primary/90 transition-colors shadow-lg"
